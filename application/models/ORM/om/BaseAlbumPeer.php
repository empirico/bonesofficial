...
abstract class BaseAlbumPeer {
	...
	const GALLERY_ID = 'album.GALLERY_ID';

	/** the column name for the COVER_PHOTO_ID field */
	const COVER_PHOTO_ID = 'album.COVER_PHOTO_ID';
	...
	const MAX_WIDTH = 'album.MAX_WIDTH';

	/** the column name for the MAX_HEIGHT field */
	const MAX_HEIGHT = 'album.MAX_HEIGHT';
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn(AlbumPeer::RANK);
			$criteria->addSelectColumn(AlbumPeer::IS_PUBLIC);
			$criteria->addSelectColumn(AlbumPeer::MAX_WIDTH);
			$criteria->addSelectColumn(AlbumPeer::MAX_HEIGHT);
		} else {
	...
	/**
	 * Returns the number of rows matching criteria, joining the related Gallery table
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct Whether to select only distinct columns; deprecated: use Criteria->setDistinct() instead.
	 * @param      PropelPDO $con
	 ...
	 * @return     int Number of matching rows.
	 */
	public static function doCountJoinGallery(Criteria $criteria, $distinct = false, PropelPDO $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		$criteria->addJoin(AlbumPeer::GALLERY_ID, GalleryPeer::ID, $join_behavior);
	...
	/**
	 * Selects a collection of Album objects pre-filled with their Gallery objects.
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	 ...
	 *		 rethrown wrapped into a PropelException.
	 */
	public static function doSelectJoinGallery(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$criteria = clone $criteria;
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			$key2 = GalleryPeer::getPrimaryKeyHashFromRow($row, $startcol);
			if ($key2 !== null) {
				$obj2 = GalleryPeer::getInstanceFromPool($key2);
				if (!$obj2) {
					...
					GalleryPeer::addInstanceToPool($obj2, $key2);
				} // if obj2 already loaded

				// Add the $obj1 (Album) to $obj2 (Gallery)
				$obj2->addAlbum($obj1);
	...
	public static function doSelectJoinAll(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		AlbumPeer::addSelectColumns($criteria);
		$startcol2 = (AlbumPeer::NUM_COLUMNS - AlbumPeer::NUM_LAZY_LOAD_COLUMNS);

		GalleryPeer::addSelectColumns($criteria);
		$startcol3 = $startcol2 + (GalleryPeer::NUM_COLUMNS - GalleryPeer::NUM_LAZY_LOAD_COLUMNS);
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			// Add objects for joined Gallery rows

			$key2 = GalleryPeer::getPrimaryKeyHashFromRow($row, $startcol2);
			if ($key2 !== null) {
				$obj2 = GalleryPeer::getInstanceFromPool($key2);
				if (!$obj2) {
				...
				// Add the $obj1 (Album) to the collection in $obj2 (Gallery)
				$obj2->addAlbum($obj1);
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		foreach ($objects as $obj) {
			...
			// delete related Photos objects
			$criteria = new Criteria(PhotosPeer::DATABASE_NAME);
			
			$criteria->add(PhotosPeer::ALBUM_ID, $obj->getId());
			$affectedRows += PhotosPeer::doDelete($criteria, $con);
		}
		return $affectedRows;
	}
