...
abstract class BasePhotos extends BaseObject  implements Persistent
{
  ...
  const PEER = 'PhotosPeer';
	...
	public function setAlbumId($v)
	{
		...
		if ($this->album_id !== $v) {
			$this->album_id = $v;
			$this->modifiedColumns[] = PhotosPeer::ALBUM_ID;
		}
	...
	public function setFileId($v)
	{
		...
		if ($this->file_id !== $v) {
			$this->file_id = $v;
			$this->modifiedColumns[] = PhotosPeer::FILE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = PhotosPeer::NUM_COLUMNS - PhotosPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(PhotosPeer::ID) ) {
					...
					$affectedRows += PhotosPeer::doUpdate($this, $con);
				}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PhotosPeer::ALBUM_ID)) $criteria->add(PhotosPeer::ALBUM_ID, $this->album_id);
		if ($this->isColumnModified(PhotosPeer::FILE_ID)) $criteria->add(PhotosPeer::FILE_ID, $this->file_id);
